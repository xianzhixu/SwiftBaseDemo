...
print("StingA:\(StringA),StringB:\(StringB)")
...
//4. 泛型类型: 能够让自定义类,结构体和枚举适用于任何类型,类似于Array和Dictionary
...
//6. 类型约束:swapTwoValues(_:_:)和Stack适用于任何类型,但有时我们需要对类型进行一些约束,这些约束可以是类型参数必须继承指定类,或者符合特定的协议或协议组合
class SomeClass {}
protocol SomeProtocol {}  // T的类型参数必须是SomeClass的子类,U的类型参数必须遵守SomeProtocol协议
func someFunction<T:SomeClass,U:SomeProtocol>(someT:T,someU:U){// 类型约束语法
    
}
// 根据字符串查找在数组中的索引
func fineIndex<T:Equatable>(of valueToFind:T,in array:[T]) ->Int?{
...
let doubleIndex = fineIndex(of: 5.0, in: [3.2,1.0,5.0,5.6])
let stringIndex = fineIndex(of: "O", in: ["V","D","E","O"])
print(doubleIndex!)
print(stringIndex!)
...
func allItemMath<C1:Container,C2:Container>(_ someContainer: C1,_ anotherContainer:C2) -> Bool where C1.ItemType == C2.ItemType,C1.ItemType: Equatable{
    // 检查两个容器是否含有相同的元素
...
if allItemMath(stackOfString, arrayOfString){
    print("全部元素匹配")
}else{
    print("元素不全部匹配")
}
